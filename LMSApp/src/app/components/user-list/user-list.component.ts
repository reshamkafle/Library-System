...
export class UserListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loading = true;
    this.userService.getUserList().pipe(first()).subscribe(users => {
      this.loading = false;
      this.users = users;
    });
    ...
    this.registerUser = this.formBuilder.group({
      ...
      libraryCardNumber: ['', Validators.required]
    });
  }
  ...
  onSubmit() {
    ...
    myUser.libraryCardNumber = this.registerUser.value.libraryCardNumber;

    let id = (<HTMLInputElement>document.getElementById("userId")).value;

    if(id === '')
    {
      this.userService.addUser(myUser).pipe(first())
      .subscribe(
        data => {
          this.ngOnInit();
        },
  ...
  updateUser(id: string){
    ...
    this.userService.getUser(id).pipe(first()).subscribe(type => {
      ...
      this.registerUser.setValue({
        ...
        libraryCardNumber: type.libraryCardNumber,  
      });
  ...
  clearModal(){
    ...
    (<HTMLInputElement>document.getElementById("libraryCardNumber")).value = '';
